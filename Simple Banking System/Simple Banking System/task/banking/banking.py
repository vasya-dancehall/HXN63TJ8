        ...
        if acc is not None:
            ...
            while True:
                menu_2 = int(
                    input("1. Balance\n2. Add income\n3. Do transfer\n4. Close account\n5. Log out\n0. Exit\n"))
                if menu_2 == 1:
                    balance = fetch_balance(card_number_input)
                    ...
                    if check is False:
                        ...
                        card_num_check = cur.fetchone()
                        if card_num_check is None:
                            print("Such a card does not exist.")
                        elif card_num_transfer == acc[0]:
                            print("You can't transfer money to the same account!")
                        else:
                            amount = int(input("Enter how much money you want to transfer:"))
                            if amount <= fetch_balance(card_number_input):

                                balance_1 = fetch_balance(card_num_transfer)
                                cur.execute(f"UPDATE card SET balance = {balance_1 + amount} WHERE number "
                                            f"= {card_num_transfer}")

                                balance_2 = fetch_balance(card_number_input)
                                cur.execute(f"UPDATE card SET balance = {balance_2 - amount} WHERE number "
                                            f"= {card_number_input}")
                                conn.commit()
                                print("Success!")
                            else:
